...
export function verifyPassword(password: string, stored: string): boolean {
  ...
  const derived = Buffer.from(hashPassword(password, Buffer.from(saltHex, 'hex')), 'hex');
...
export async function createSession(userId: string) {
  ...
  await prisma.session.create({ data: { id: randomBytes(16).toString('hex'), tokenHash: tokenHash(token), userId, expiresAt } });
...
export async function getCurrentUser(): Promise<AuthUser | null> {
  ...
  if (session.expiresAt <= new Date()) { await prisma.session.delete({ where: { id: session.id } }).catch(() => undefined); return null; }

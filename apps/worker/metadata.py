...
def generate_metadata(clips: list[dict[str, Any]], language: str) -> list[dict[str, Any]]:
    ...
    for index, clip in enumerate(clips):
        payload.append({'index': index, 'title': clip.get('title', ''), 'hook': clip.get('hook', ''), 'category': clip.get('category', 'Other'), 'score': clip.get('score', 0)})
    ...
    for index, clip in enumerate(clips):
        ...
        output.append({
            **clip,
            ...
            'youtubeTitle': item.get('youtubeTitle', clip.get('title', '')),
            'instagramCaption': item.get('instagramCaption', ''),
            'tiktokCaption': item.get('tiktokCaption', ''),
            'metadataStatus': 'generated' if item else 'fallback',
        })
    return output

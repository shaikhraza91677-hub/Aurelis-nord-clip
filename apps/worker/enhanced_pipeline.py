...
from worker import OPENROUTER_MODEL, audio, clip_focus_x, download, moments, transcribe
...
def _caption_transcript(wav: Path, original: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        model = WhisperModel(
            __import__('os').getenv('WHISPER_MODEL', 'small'),
            device=__import__('os').getenv('WHISPER_DEVICE', 'cpu'),
...
def process_url(url: str, outdir: str = './output') -> dict[str, Any]:
    root = Path(outdir)
    root.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix='aurelis-production-') as td:
        work = Path(td)
        video = download(url, work)
        return _process_video(video, url, root, work)


def process_file(source: str, outdir: str = './output') -> dict[str, Any]:
    video = Path(source).resolve()
    if not video.exists() or not video.is_file():
        raise RuntimeError('Uploaded media file does not exist')
    root = Path(outdir)
    root.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix='aurelis-production-upload-') as td:
        work = Path(td)
        return _process_video(video, f'upload://{video.name}', root, work)


def _process_video(video: Path, source: str, root: Path, work: Path) -> dict[str, Any]:
    wav = work / 'audio.wav'
    audio(video, wav)
    ...
    for index, clip in enumerate(clips[:8], 1):
        ...
        rendered.append({
            **clip,
            'file': str(target),
            'captionLanguage': 'hinglish' if str(transcript.get('language', '')).lower().startswith('hi') else 'english',
            'hookTransliterated': transliterate_word(str(clip.get('hook', '')), str(transcript.get('language', ''))),
            'framing': {'mode': crop_plan.get('mode', 'center'), 'focusX': clip_focus_x(crop_plan, start, end)},
        })
    rendered = generate_metadata(rendered, str(transcript.get('language', 'en')))
    manifest = {'source': source, 'language': transcript.get('language'), 'transcript': transcript, 'cropPlan': crop_plan, 'clips': rendered, 'model': OPENROUTER_MODEL}

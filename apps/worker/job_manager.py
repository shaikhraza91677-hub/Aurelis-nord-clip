...
_LOCK=threading.RLock();_JOBS:dict[str,dict[str,Any]]={}
...
try:
    ...
    if os.getenv('REDIS_URL'):
        _REDIS=redis.Redis.from_url(os.getenv('REDIS_URL'),decode_responses=True)
        _REDIS.ping();_REDIS_ENABLED=True
except Exception:
    _REDIS=None;_REDIS_ENABLED=False
...
def submit(kind:str,payload:dict[str,Any],local_fn:Callable[[],dict[str,Any]]|None=None)->str:
    ...
    if _REDIS_ENABLED:_REDIS.rpush('aurelis:queue',job_id)
    elif local_fn:_POOL.submit(_run_local,job_id,local_fn)
    else:raise RuntimeError('Redis queue is unavailable and no local runner was provided')
    return job_id

def _handler(kind,payload):
    if kind=='analyze-url':
        from enhanced_pipeline import process_url
        return process_url(payload['url'],payload.get('out','./output'),payload.get('prompt',''))
    if kind=='analyze-file':
        from enhanced_pipeline import process_file
        return process_file(payload['path'],payload.get('out','./output'),payload.get('prompt',''))
    if kind=='render-clip':
        from render import render_clip
        return render_clip(payload['sourceUrl'],float(payload['start']),float(payload['end']),payload['out'],payload.get('config'))
    if kind=='supercut':
        from supercut import create_supercut
        return create_supercut(payload.get('files',[]),payload['out'],payload.get('transition','hard'))
    raise RuntimeError(f'Unknown job kind: {kind}')

def _run_local(job_id,fn):
    _run(job_id,fn)
...
def run_redis_worker_forever():
    ...
    while True:
        ...
        _,job_id=item;job=get(job_id)
        if not job or job.get('status') not in ('queued','processing'):continue
        _run(job_id,lambda:_handler(job['kind'],job.get('payload',{})))
...
def list_jobs(limit=20):
    if _REDIS_ENABLED:
        ids=_REDIS.lrange('aurelis:recent',0,max(0,limit-1)) if _REDIS.exists('aurelis:recent') else []
    else:ids=[]
    with _LOCK:items=list(_JOBS.values())
    if not items and _REDIS_ENABLED:return []

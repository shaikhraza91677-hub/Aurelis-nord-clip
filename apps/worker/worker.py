...
def transcribe(path: Path) -> dict[str, Any]:
    """Use faster-whisper locally when installed; otherwise fail with a clear setup message.

    Keeping transcription local avoids depending on a paid speech API and gives us the
    word timestamps needed for animated captions.
    """
    try:
...
def normalize_caption(text: str, language: str) -> str:
    """Hindi captions are intentionally returned in Latin-script Hinglish for the MVP.
    Other languages stay in their original language unless the project explicitly asks for English.
    """
...
def ass_caption(words: list[dict[str, Any]], start: float, end: float, out: Path) -> None:
    selected = [w for w in words if w["end"] >= start and w["start"] <= end]
    selected = [{**w, "start": max(0.0, w["start"] - start), "end": max(0.0, w["end"] - start)} for w in selected]
    out.write_text(json.dumps(selected, ensure_ascii=False, indent=2), encoding="utf-8")


def render(video: Path, out: Path, start: float, end: float, caption_file: Path | None = None) -> None:
    duration = max(1.0, end - start)
    # Center crop is a safe baseline. Speaker tracking will replace this crop plan later.
    vf = "scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920"
    if caption_file:
        # The actual word-pop renderer is separate; keeping the caption timeline next to the render
        # makes it possible to swap styles without re-transcribing.
        pass
    run([
...
def process(url: str, outdir: str = "./output") -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="aurelis-") as td:
        ...
        for i, clip in enumerate(clips[:8], 1):
            ...
            caption_path = root / f"clip-{i:02d}.words.json"
            ass_caption(transcript["words"], start, end, caption_path)
            target = root / f"clip-{i:02d}.mp4"
            render(video, target, start, end, caption_path)
            rendered.append({**clip, "file": str(target), "captionTimeline": str(caption_path), "captions": normalize_caption(str(clip.get("hook", "")), str(transcript.get("language", "")))})
        manifest = {
            "source": url,
